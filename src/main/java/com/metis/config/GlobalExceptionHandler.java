...
//        "/"是根路径之意;如果使用"error/500"则是相对路径之意,会接在上次请求"相对根路径"的后面
    ...
    /**
     * 1. todo 内部类被编译后等同于另一个类,但是为什么会优先匹配内部类中的mapper? 2. @ControllerAdvice
     * 不会作用到内部类中,大概因为动态代理不会针对内部类生成代理对象？-
     */
    ...
    public static class AdaptiveExceptionHandler{
        ...
        public JsonResult<?> handleHttpMessageNotReadableException(MissingServletRequestParameterException ex) {
            ...
            return new JsonResult<>(BusinessStatusEnum.LACK_PARAM.getCode(), BusinessStatusEnum.LACK_PARAM.getDesc());
        }
        ...
        public JsonResult<?> handleTypeMismatchException(NullPointerException ex) {
            ...
            return new JsonResult<>(ExceptionMsgEnum.NULL_VALUE.getCode(), ExceptionMsgEnum.NULL_VALUE.getMsg());
        }
        ...
        @ResponseStatus(value = HttpStatus.INTERNAL_SERVER_ERROR)
        public JsonResult<?> handleBusinessError(NonBusinessRuntimeException ex) {
